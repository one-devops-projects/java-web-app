pipeline{
    ...
    stages{
        ...
        stage('Pull and run image from AWS ECR'){
		    ...
		    sh "docker rm -f maven-web-app || true"
                    sh "docker run -d --name maven-web-app -p 8080:8080 537960581474.dkr.ecr.us-east-1.amazonaws.com/java-web-app:${BUILD_NUMBER}"
                }
            }
        }
